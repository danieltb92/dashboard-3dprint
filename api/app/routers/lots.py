...
from ..schemas import CostBreakdown, LotCalculate, ProductionLotCreate, ProductionLotRead
...
def get_cost_settings(db: Session) -> dict[str, int | float]:
    ...
    return {
        ...
        "margen_default": int(saved.get("margen_default", app_settings.default_margin_pct)),
        "prep_min": float(saved.get("prep_min", 0)),
        "post_min": float(saved.get("post_min", 0)),
    }
...
def calculate(payload: LotCalculate, db: Session) -> dict[str, int]:
    ...
    if not printer.activa or not material.activa:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="La impresora y el material deben estar activos")
    return calculate_production_cost(printer, material, get_cost_settings(db), payload.peso_g, payload.minutos, payload.qty)
...
def create_lot(payload: ProductionLotCreate, db: Session = Depends(get_db)):
    ...
    item = ProductionLot(
        **payload.model_dump(), horas=minutes / 60, desglose_json=breakdown,
